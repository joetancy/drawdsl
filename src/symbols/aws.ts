...
function resource(icon: string, fill = "#527FFF", stroke = "#ffffff"): SymbolDefinition {
  return {
    role: "resource",
    drawio: { shape: `${AWS4}.resourceIcon`, resIcon: `${AWS4}.${icon}`, fill, stroke },
  };
}
...
function group(fill: string, stroke: string, icon: string, fontColor: string, suppressBorder = false): SymbolDefinition {
  return {
    role: "container",
    drawio: {
      shape: `${AWS4}.group`,
      fill,
      stroke,
      styles: [`grIcon=${AWS4}.${icon}`, ...(suppressBorder ? ["grStroke=0"] : []), `fontColor=${fontColor}`],
    },
  };
}
...
const symbols: Record<string, SymbolDefinition> = {
  ...
  private_subnet: group("#E6F6F7", "#00A4A6", "group_security_group", "#147EBA", true),
  public_subnet: group("#F2F6E8", "#7AA116", "group_security_group", "#248814", true),
  az: group("none", "#147EBA", "group_availability_zone", "#147EBA"),
  internet: shape("internet", "#232F3D", "none", { widthScale: 1, heightScale: 0.6 }),
  ...
  illustration_desktop: shape("illustration_desktop", "#000000"),
  illustration_users: shape("illustration_users", "#000000"),
